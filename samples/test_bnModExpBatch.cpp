...
//               BN_num_bytes(bn_mod), BN_num_bits(bn_mod));
	...
	unsigned char* bn_base_data_ =
            (unsigned char*)calloc(len_ * BATCH_SIZE, sizeof(unsigned char));
        if (NULL == bn_base_data_) exit(1);
	for (unsigned int i = 0; i < BATCH_SIZE; i++)
        BN_bn2binpad(bn_base, bn_base_data_+len_*i, len_);
        unsigned char* bn_mod_data_ =
            (unsigned char*)calloc(len_ * BATCH_SIZE, sizeof(unsigned char));
        if (NULL == bn_mod_data_) exit(1);
	for (unsigned int i = 0; i < BATCH_SIZE; i++)
        BN_bn2binpad(bn_mod, bn_mod_data_+len_*i, len_);
        unsigned char* bn_exponent_data_ =
            (unsigned char*)calloc(len_ * BATCH_SIZE, sizeof(unsigned char));
        if (NULL == bn_exponent_data_) exit(1);
	for (unsigned int i = 0; i < BATCH_SIZE; i++)
        BN_bn2binpad(bn_exponent, bn_exponent_data_+len_*i, len_);
	...
	// Simulate input number in BigNumber representation
        ...
        std::vector<BigNumber> big_num_base(BATCH_SIZE);
        std::vector<BigNumber> big_num_mod(BATCH_SIZE);
        std::vector<BigNumber> big_num_exponent(BATCH_SIZE);

	for (unsigned int i = 0; i < BATCH_SIZE; i++) {
	status = binToBigNumber(big_num_base[i], bn_base_data_ + len_*i, bit_length);
        if (HE_QAT_STATUS_SUCCESS != status) {
            printf("Failed at binToBigNumber()\n");
            exit(1);
        }
        status = binToBigNumber(big_num_mod[i], bn_mod_data_ + len_*i, bit_length);
        if (HE_QAT_STATUS_SUCCESS != status) {
            printf("Failed at binToBigNumber()\n");
            exit(1);
        }
        status = binToBigNumber(big_num_exponent[i], bn_exponent_data_ + len_*i, bit_length);
        if (HE_QAT_STATUS_SUCCESS != status) {
            printf("Failed at binToBigNumber()\n");
            exit(1);
        }
	...
	// Reset numbers to 0
        memset(bn_base_data_ + len_*i, 0, len_);
        memset(bn_mod_data_ + len_*i, 0, len_);
        memset(bn_exponent_data_ + len_*i, 0, len_);
        // Make sure variables are reset
	if (memcmp(bn_base_data_ + len_*i, bn_mod_data_ + len_*i, len_) || 
			memcmp(bn_base_data_ + len_*i, bn_exponent_data_ + len_*i, len_)) {
	    PRINT_ERR("Pointers are not reset to zero!");
	    exit(1);
	}

        start = clock();
	status = bigNumberToBin(bn_base_data_ + len_*i, bit_length, big_num_base[i]);
        if (HE_QAT_STATUS_SUCCESS != status) {
            printf("bn_base_data_: failed at bignumbertobin()\n");
            exit(1);
        }
	status = bigNumberToBin(bn_mod_data_ + len_*i, bit_length, big_num_mod[i]);
        if (HE_QAT_STATUS_SUCCESS != status) {
            printf("bn_base_data_: failed at bignumbertobin()\n");
            exit(1);
        }
	status = bigNumberToBin(bn_exponent_data_ + len_*i, bit_length, big_num_exponent[i]);
        if (HE_QAT_STATUS_SUCCESS != status) {
            printf("bn_base_data_: failed at bignumbertobin()\n");
            exit(1);
        }
	...
	// Perform BigNumber modular exponentiation on QAT
        start = clock();
	for (unsigned int i = 0; i < BATCH_SIZE; i++) 
        status = HE_QAT_bnModExp(bn_remainder_data_ + len_*i, bn_base_data_ + len_*i, 
			bn_exponent_data_ + len_*i, bn_mod_data_ + len_*i, bit_length);
