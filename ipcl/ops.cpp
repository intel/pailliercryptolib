...
// constructors
//
EncryptedNumber::EncryptedNumber(const PublicKey* pub_key, const BigNumber& bn)
...
EncryptedNumber::EncryptedNumber(const PublicKey* pub_key,
...
EncryptedNumber EncryptedNumber::operator+(const EncryptedNumber& other) const {
  ...
  if (m_available == 1) {
    ...
    return EncryptedNumber(m_pubkey, sum);
  }
...
// CT+PT
EncryptedNumber EncryptedNumber::operator+(const BigNumber& other) const {
  const auto& a = *this;
  BigNumber b;
...
EncryptedNumber EncryptedNumber::operator+(
...
// CT*PT EncryptedNumber store a plaintext integer, not an encrypted
// integer
EncryptedNumber EncryptedNumber::operator*(const EncryptedNumber& other) const {
  ERROR_CHECK(m_pubkey->getN() == other.m_pubkey->getN(),
              "operator*: two different public keys detected!!");
  ...
  if (m_available == 1) {
    ...
    return EncryptedNumber(m_pubkey, product);
  }
...
EncryptedNumber EncryptedNumber::operator*(const BigNumber& other) const {
  ...
  BigNumber b = other;
  BigNumber product = a.raw_mul(a.m_bn.front(), b);
  return EncryptedNumber(m_pubkey, product);
}

BigNumber EncryptedNumber::raw_add(const BigNumber& a,
...
std::vector<BigNumber> EncryptedNumber::raw_mul(
...
BigNumber EncryptedNumber::raw_mul(const BigNumber& a,
                                   const BigNumber& b) const {
  const BigNumber& sq = m_pubkey->getNSQ();
  return ipcl::ippModExp(a, b, sq);
}

EncryptedNumber EncryptedNumber::rotate(int shift) const {
  ERROR_CHECK(m_available != 1, "rotate: Cannot rotate single EncryptedNumber");
  ERROR_CHECK(shift >= -8 && shift <= 8,
              "rotate: Cannot shift more than 8 or -8");

  if (shift == 0 || shift == 8 || shift == -8)
    return EncryptedNumber(m_pubkey, m_bn);
  ...
  return EncryptedNumber(m_pubkey, new_bn);
}

void EncryptedNumber::apply_obfuscator() {
  b_isObfuscator = true;
