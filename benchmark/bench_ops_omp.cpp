...
static void BM_Add_CTCT_OMP(benchmark::State& state) {
  ...
  std::size_t v_size = (dsize + BATCH_SIZE - 1) / BATCH_SIZE;

  // init pub key & priv key
  ...
  // init some big numbers, which need to be encrypted
  std::vector<BigNumber> exp_bn1_v(BATCH_SIZE), exp_bn2_v(BATCH_SIZE);
  for (int i = 0; i < BATCH_SIZE; i++) {
    exp_bn1_v[i] = P_BN - BigNumber((unsigned int)(i * 1024));
    exp_bn2_v[i] = Q_BN + BigNumber((unsigned int)(i * 1024));
  }

  // init plaintext with given big numbers
  std::vector<ipcl::PlainText> pt1_v(v_size, ipcl::PlainText(exp_bn1_v));
  std::vector<ipcl::PlainText> pt2_v(v_size, ipcl::PlainText(exp_bn2_v));
  // set R & HS with a fixed value
  std::vector<BigNumber> r_bn_v(BATCH_SIZE, R_BN);
  ...
  for (std::size_t i = 0; i < v_size; i++) {
    ct1_v[i] = pub_key->encrypt(pt1_v[i]);
    ct2_v[i] = pub_key->encrypt(pt2_v[i]);
  }

  std::vector<ipcl::CipherText> sum(v_size);
  for (auto _ : state) {
// do CT+CT
...
static void BM_Mul_CTPT_OMP(benchmark::State& state) {
  ...
  std::vector<ipcl::CipherText> product(v_size);
  for (auto _ : state) {
// do CT*PT
#ifdef IPCL_BENCHMARK_USE_OMP
#pragma omp parallel for
