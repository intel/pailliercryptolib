...
static void BM_Encrypt_OMP(benchmark::State& state) {
  ...
  std::vector<BigNumber> exp_bn_v(BATCH_SIZE);
  for (size_t i = 0; i < BATCH_SIZE; i++) {
    exp_bn_v[i] = P_BN - BigNumber((unsigned int)(i * 1024));
  }

  // init plaintext with given big numbers
  std::vector<ipcl::PlainText> pt_v(v_size, ipcl::PlainText(exp_bn_v));
...
static void BM_Decrypt_OMP(benchmark::State& state) {
  ...
  std::vector<ipcl::PlainText> dt_v(v_size);
  for (auto _ : state) {
// do decrypt
#ifdef IPCL_BENCHMARK_USE_OMP
#pragma omp parallel for
#endif  // IPCL_BENCHMARK_USE_OMP
    for (std::size_t i = 0; i < v_size; i++) {
      dt_v[i] = priv_key->decrypt(ct_v[i]);
    }
  }
